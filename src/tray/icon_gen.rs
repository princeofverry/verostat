...
/// Generates the default circular VeroStat "V" logo icon.
pub fn generate_logo_icon() -> Result<Icon, String> {
    let width = 32u32;
    let height = 32u32;
    let mut rgba = vec![0u8; (width * height * 4) as usize];
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if dist <= 14.5 {
                ...
                let on_left_arm = (y >= 9 && y <= 23) && {
                    let expected_x = 8.0 + (y as f32 - 9.0) * (8.0 / 14.0);
                    (x as f32 - expected_x).abs() <= 1.6
                };

                let on_right_arm = (y >= 9 && y <= 23) && {
                    let expected_x = 24.0 - (y as f32 - 9.0) * (8.0 / 14.0);
                    (x as f32 - expected_x).abs() <= 1.6
                };

                let on_bar = (y == 8 || y == 9) && (x >= 13 && x <= 19);

                if on_left_arm || on_right_arm || on_bar {
                    rgba[idx] = 56;
                    rgba[idx + 1] = 189;
                    rgba[idx + 2] = 248;
                    rgba[idx + 3] = 255;
    ...
    Icon::from_rgba(rgba, width, height).map_err(|e| format!("Failed to create logo icon: {:?}", e))
}
